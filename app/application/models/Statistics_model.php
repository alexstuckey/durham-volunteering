...
class Statistics_model extends CI_Model {
    ...
    {
        $query = $this->db->query("SELECT cisID, SEC_TO_TIME( SUM( TIME_TO_SEC( TIMEDIFF(finish,start) ) ) ) AS timeSum FROM times WHERE cisID=" . $CISID ." AND status='completed' ");
    ...
    {
        $query = $this->db->query("SELECT SEC_TO_TIME( SUM( TIME_TO_SEC( TIMEDIFF(finish,start) ) ) ) AS timeSum FROM times WHERE status='completed' ");
    ...
    {
        $query = $this->db->query("SELECT cisID, organisation, SEC_TO_TIME( SUM( TIME_TO_SEC( TIMEDIFF(finish,start) ) ) ) AS timeSum  FROM times JOIN causes WHERE times.causeID=causes.causeID AND status='completed' AND ". $cisID . "=times.cisID  GROUP BY causes.organisation order by timeSum desc limit 1");
    ...
    public function positionWithinDepartment($cisID)
    {
        ...
        $query = $this->db->query("SELECT users.cisID,departmentsName, SEC_TO_TIME( SUM( TIME_TO_SEC( TIMEDIFF(finish,start) ) ) ) AS timeSum FROM times join users ON times.cisID=users.cisID join departments ON users.departmentID=departments.ID WHERE times.status='completed' AND departmentsName=\"" .$department."\" group by cisID,departmentsName ORDER BY timeSum desc");
